...
const SystemDateCard = ({ systemDate, currentDate, onEndOfDayClick }: SystemDateCardProps) => {
    ...
    // systemDate'in currentDate'den büyük olup olmadığını kontrol et
    const isSystemDateNextDay = systemDate > currentDate;
        ...
        <div className="group relative bg-white border border-gray-200 rounded-3xl p-4 shadow-lg hover:shadow-xl transition-colors duration-300 w-96 max-w-md backdrop-blur-sm">
            ...
            <div className="relative z-10">
                <button
                    ...
                    disabled={isSystemDateNextDay}
                    className={`group/btn w-full relative overflow-hidden rounded-2xl py-2 px-4 font-bold text-lg shadow-lg transition-all duration-300 focus:outline-none focus:ring-4 ${isSystemDateNextDay
                        ? 'bg-gray-400 text-gray-600 cursor-not-allowed shadow-md'
                        : 'bg-gradient-to-r from-green-500 via-green-600 to-emerald-600 text-white hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] focus:ring-green-200 cursor-pointer'
                        }`}
                >
                    <div className={`absolute inset-0 transition-opacity duration-300 ${isSystemDateNextDay
                        ? 'bg-gray-300 opacity-0'
                        : 'bg-gradient-to-r from-green-400 to-emerald-500 opacity-0 group-hover/btn:opacity-100'
                        }`} />
                    <div className="relative flex items-center justify-center gap-3">
                        <svg className={`w-5 h-5 transition-transform ${isSystemDateNextDay ? '' : 'group-hover/btn:rotate-12'
                            }`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span>{t('report.systemDate.endOfDayButton')}</span>
                        <svg className={`w-4 h-4 transition-transform ${isSystemDateNextDay ? '' : 'group-hover/btn:translate-x-1'
                            }`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
